...
```
agno[openai]            # Agno framework + OpenAI model support
```

> **Important:** The package is `agno` (imports: `from agno.agent import Agent`,
> `from agno.models.openai import OpenAIChat`). Install with the `[openai]`
> extra so a compatible `openai` version is pulled in automatically. Do **not**
> pin an old `openai` version — modern Agno requires a recent one.
